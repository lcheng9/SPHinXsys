...
unvMesh2412::UNVRecordData::UNVRecordData():
  label(-1),
  fe_descriptor_id(-1),
  phys_prop_tab_num(2),
  mat_prop_tab_num(-1),
...
void unvMesh2412::read_stream(std::ifstream& in_stream, TDataSet& theDataSet)
{
  ...
  while( !in_stream.eof())
  {
    ...
    in_stream>>aRec.mat_prop_tab_num; // part Id
...
void unvMesh2412::write_stream(std::ofstream& out_stream, const TDataSet& theDataSet)
{
  ...
  for(; anIter != theDataSet.end(); anIter++)
  {
    ...
    out_stream<<std::setw(6)<<aRec.fe_descriptor_id;  /* type of element */
    out_stream<<std::setw(6)<<aRec.phys_prop_tab_num;
    out_stream<<std::setw(6)<<aRec.mat_prop_tab_num;
    out_stream<<std::setw(6)<<aRec.color;
    out_stream<<std::setw(6)<<aRec.node_labels.size()<<std::endl;  /* No. of nodes per element */
...
bool unvMesh2412::is_face_element(int theFeDescriptorId){
...
bool unvMesh2412::is_body_element(int theFeDescriptorId){
  //if(!is_beam_element(theFeDescriptorId) && !is_face_element(theFeDescriptorId))
  //  return true;
  switch (theFeDescriptorId){
